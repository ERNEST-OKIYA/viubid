...
def get_uniques(bid_id):
    ...
    unique_bids = UserBid.objects.filter(bid_entry__bid__id=bid_id).values('amount','bid_entry__user__first_name').annotate(amount_count=Count('amount')).filter(amount_count__lt=2).order_by('amount')
