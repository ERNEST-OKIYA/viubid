...
class UniqueBids(View):
	...
	def get(self,request,bid_id):
		...
		for ub in unique_bids:
			...
			entry = UserBid.objects.filter(bid_entry__bid__id=bid_id).get(amount=amount).order_by('amount')
		...
		return render(request,'admin_portal/unique-bids.html',{'data':datalist})
	def post(self,request):
		pass
